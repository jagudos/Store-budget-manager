...
#jq --arg timestamp "$timestamp" '.telefon = listin_$timestamp' /usr/share/validador/config.json
jq --arg collectionName "$collectionName" '.telefon = $collectionName' /usr/share/validador/config.json
